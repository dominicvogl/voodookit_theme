...
//
// Author: Dominic Vogl
// Description: Various functions for rendering images in Wordpress and add SVG support
// ----------------------------------------------------------------------------------------------------------------------
...
function cccc_get_image($attachmentID, $size_args = 'thumbnail', $is_lazy = false, $echo = true, $elem_args = array() ) {

	// Get path to the image
	$html = '';

	// Get alt attribute for image tag
	$attachment_attributes = cccc_get_attachment($attachmentID);
	...
	if( is_array($size_args) && $elem_args["image_behavior"] != 'straight') {
		...
		//$sizes = cccc_get_image_sizes($size_args[0]["image_namespace"]);
		...
		if($image_src) {
			...
			// Get image sizes
			$sizes = cccc_get_image_sizes($size_args);

			if($sizes == false) {
				$sizes = cccc_get_image_sizes('thumbnail');
			}
...
function cccc_get_attachment($attachment_id) {
...
function cccc_get_image_sizes( $size = '' ) {
...
function get_srcset_image($image_ID, $image_sizes = array()) {
